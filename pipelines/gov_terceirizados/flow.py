...
#PIPELINE DATE
REF_DATE = datetime(2025, 1, 1).strftime("%Y-%m")
...
def export_to_gcs(model_name:str,schema:str,path_to_parquet:str):
    ...
    try:
        con.execute(f"COPY main_{schema}.{model_name} TO '{path_to_parquet}' (FORMAT PARQUET)")
...
def run_dbt_commands(commands: list[list[str]], vars: dict | None = None) -> None:
    ...
    if dotenv.load_dotenv("/app/.env"):
        ...
        raise ValueError("Não foi possível carregar as variáveis de ambiente do arquivo .env")
...
def dbt_run_bronze(partition: str = "*"):
    if partition == "*":
        ...
        parquet_path = f"terceirizados_{partition}.csv"
    ...
    export_to_gcs(model_name="brutos_terceirizados", schema="bronze", path_to_parquet=str(BRONZE_BUCKET + f"/brutos_tercerizados.parquet"))
...
def dbt_run_silver_dims(dimensions_dir: Path):
   ...
   for sql_file in sql_files:
        ...
        run_dbt_commands(
            commands=[["run", "--select", model_name]]
        )

        export_to_gcs(model_name=model_name, schema="prata", path_to_parquet=str(SILVER_BUCKET +f"/{model_name}" + f"/{model_name}.parquet"))
...
def dbt_run_gold():
    run_dbt_commands(
        commands=[["run", "--select", "mart_terceirizados"]]
        )
    
    export_to_gcs(model_name="mart_terceirizados", schema="gold", path_to_parquet=str(GOLD_BUCKET + f"/mart_terceirizados.parquet"))


@flow(name="terceirizados-dbt-pipeline")
